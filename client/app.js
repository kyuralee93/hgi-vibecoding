...
(function(){
  ...
  window.loadLayerAdminForm = function(){
    ...
    q('admLayerName').value=l.layer||''; q('admLayerFrom').value=num(l.from); q('admLayerTo').value=num(l.to); q('admLayerLead').value=l.lead||''; q('admLayerParticipants').value=l.participants||defaultParticipants(t.treatyId,l.lead); if(q('admLayerAp')) q('admLayerAp').value=(l.ap!=null&&l.ap!=='')?num(l.ap):(window.graDefaultLayerAp?window.graDefaultLayerAp(i,num(l.to)-num(l.from)):0);
  };
  ...
  window.saveTreatyAdminLayer = function(){
    ...
    l.lead=q('admLayerLead').value.trim()||'TBD'; l.participants=q('admLayerParticipants').value.trim()||defaultParticipants(t.treatyId,l.lead); if(q('admLayerAp')) l.ap=num(q('admLayerAp').value);
  ...
  function renderReinsAdminTable(){
    ...
    box.innerHTML=getTreaties().map(t=>`<div class="admin-program-row" style="--treaty-accent:${colorFor(t.treatyId)}"><h4>${esc(t.name)} <span>${esc(t.type)}</span></h4><table><thead><tr><th>Layer</th><th>구간</th><th>연간재보험료</th><th>Lead</th><th>참여사</th></tr></thead><tbody>${(t.layers||[]).map((l,li)=>`<tr><td>${esc(l.layer)}</td><td>${fmtEok(l.from)} ~ ${fmtEok(l.to)}</td><td>${fmtEok((l.ap!=null&&l.ap!=='')?num(l.ap):(window.graDefaultLayerAp?window.graDefaultLayerAp(li,num(l.to)-num(l.from)):0))}</td><td>${esc(l.lead||'-')}</td><td>${esc(participantsText(l))}</td></tr>`).join('')}</tbody></table></div>`).join('');
  }
