...
## v52 누적위험 조회 — 최신 벡터 지도 코로플레스 (riskmap.js)
- 기존 Leaflet/OSM 지도를 **MapLibre GL 벡터 지도 + CARTO 벡터 베이스맵**으로 교체(API 키 불필요)
- 최초 세계 지도에서 **대륙별 누적 계약금액**을 색으로 표시(금액 클수록 빨강, 적을수록 연함)
- 마우스오버: 해당 영역 테두리 강조 + 툴팁(총 건수, 국가별 건수/금액)
- 적정 배율로 확대 시 **국가별 색상**으로 전환, 툴팁 세부도 **도시별**로 전환
- app.js의 renderMap을 오버라이드(기존 지역 카드/Drill-down 재사용). 원하면 키만 넣어 Mapbox/Google로 교체 가능
...
누적위험 지도는 인터넷 연결 시 지도 타일/경계 데이터를 CDN에서 불러옵니다.
